...
if st.sidebar.button("Загрузить и предсказать"):
    with st.spinner("Загружаем данные..."):
        ...
        st.caption("Прогноз каждой модели сравнивается с реальным движением на следующий день")

        history_rows = []
        history_days = min(8, len(df_xgb) - 1)
        ...
        with st.spinner("Считаем историю прогнозов..."):
            for i in range(-history_days, -1):
                ...
                # Chronos прогноз
                ctx = torch.tensor(
                    df_xgb["Price"].values[:i if i != 0 else None][-context_days:],
                    dtype=torch.float32
                )
                ...
                history_rows.append({
                    ...
                    "XGBoost точность": xgb_ok,
                    "Chronos прогноз": f"${chr_pred:.2f} {chr_dir}",
                    "Chronos точность": chr_ok,
                })

        hist_df = pd.DataFrame(history_rows)
        st.dataframe(hist_df, use_container_width=True, hide_index=True)

        # Итоговая точность за неделю
        xgb_acc_week  = sum(1 for r in history_rows if r["XGBoost точность"] == "✅") / len(history_rows) * 100
        chr_acc_week  = sum(1 for r in history_rows if r["Chronos точность"] == "✅") / len(history_rows) * 100

        col1, col2, col3 = st.columns(3)
        col1.metric("XGBoost точность (7д)", f"{xgb_acc_week:.0f}%")
        col2.metric("Chronos точность (7д)", f"{chr_acc_week:.0f}%")
        col3.metric("Дней в выборке", len(history_rows))
