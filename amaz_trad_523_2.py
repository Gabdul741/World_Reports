...
if st.sidebar.button("Загрузить и предсказать"):
    with st.spinner("Загружаем данные..."):
        ...
        # Строим список торговых дней: 4 прошлых + сегодня + 2 будущих
        trading_days = list(df.index)
        today_idx = len(trading_days) - 1

        window = []
        # 4 прошлых торговых дня
        for offset in range(-4, 0):
            idx = today_idx + offset
            if idx >= 0:
                window.append(trading_days[idx])
        # Сегодня
        window.append(trading_days[today_idx])
        # 2 будущих рабочих дня (пн-пт)
        last = window[-1]
        for _ in range(2):
